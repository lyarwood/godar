...
func (an *AppletNotifier) Send(callsign, aircraftType string, altitude int, speed float64, distance float64, direction string, previousDistance ...float64) error {
	// Add to applet's recent aircraft list
	an.applet.AddAircraftDetection(callsign, aircraftType, altitude, distance)

	// Send the actual notification
	return an.notifier.Send(callsign, aircraftType, altitude, speed, distance, direction, previousDistance...)
}
